					   ...
					   int x, int y, int rect_width, int rect_height, int type)
{
	...
	//TODO
	cv::Point a = cv::Point(x, y), b = cv::Point(x + rect_width, y + rect_height);

	cv::Mat aux_mask(img_height, img_width, type, cv::Scalar::all(0));
	mask = aux_mask.clone();

	cv::rectangle(mask, a, b, cv::Scalar::all(255), CV_FILLED);
	//
					 ...
					 int x, int y, int radius, int type)
{
	...
	cv::circle(mask, cv::Point(x, y), radius, cv::Scalar::all(255), CV_FILLED);
	//
