...
fsiv_filter2D(cv::Mat const& in, cv::Mat const& filter)
{
	...
	for (int i = 0; i < r; ++i)
		for (int j = 0; j < c; ++j)
			ret_v.at<float>(i,j) = sum(filter.mul(in(cv::Rect(j,i,filter.cols,filter.rows))));
	//
	CV_Assert(ret_v.type()==CV_32FC1);
...
fsiv_extend_image(const cv::Mat& img, const cv::Size& new_size)
{
	...
	cv::Mat out;
	//TODO
	int d = new_size.width - img.cols;
	out   = fsiv_fill_expansion(img,d/2);
	//
